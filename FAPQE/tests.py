...
def attack_local_models(args, protection):
    ...
    if protection:
        for img_path in glob.glob(os.path.join(result_dir, "*.png")):
            ...
            # Try to find the original image with different extensions
            clean_img_path = None
            possible_extensions = ['.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG']

            # First try exact match
            for ext in possible_extensions:
                test_path = os.path.join(args.source_dir, base_name + ext)
                if os.path.exists(test_path):
                    clean_img_path = test_path
                    break

            # If no exact match, try to handle suffix mismatch
            # Handle cases like:
            # - image_0001_2.png -> image_0001_02.jpg (old format)
            # - image_0001_02.png -> image_0001_02.jpg (new format, should match exactly)
            if clean_img_path is None and '_' in base_name:
                # For new format (image_0001_02), try exact match first
                for ext in possible_extensions:
                    test_path = os.path.join(args.source_dir, f"{base_name}{ext}")
                    if os.path.exists(test_path):
                        clean_img_path = test_path
                        break

                # If still no match, try suffix conversion for old format
                if clean_img_path is None:
                    # Extract the base part without suffix
                    base_part = base_name.rsplit('_', 1)[0]  # image_0001
                    suffix_part = base_name.rsplit('_', 1)[1]  # 2 or 02

                    # Try different suffix formats
                    possible_suffixes = [
                        suffix_part.zfill(2),  # 2 -> 02
                        suffix_part,           # keep as is
                        '1',                   # fallback to 1
                        '01'                   # fallback to 01
                    ]
                    ...
                    for suffix in possible_suffixes:
                        for ext in possible_extensions:
                            test_path = os.path.join(args.source_dir, f"{base_part}_{suffix}{ext}")
                            if os.path.exists(test_path):
                                clean_img_path = test_path
                                break
                        if clean_img_path:
            ...
            # Check if clean image was loaded successfully
            if clean_img is None:
                print(f"Warning: Could not load clean image from {clean_img_path}")
                continue
            ...
            combined_fn = f"{base_name}_cos_simi_{cos_simi.item():.5f}.png"
    ...
    # Ensure floating point division and higher precision
    psr_far01 = float(FAR01) / float(total) if total > 0 else 0.0
    psr_far001 = float(FAR001) / float(total) if total > 0 else 0.0
    psr_far0001 = float(FAR0001) / float(total) if total > 0 else 0.0

    result_str = f"{args.test_model_name[0]} PSR in FAR@0.1: {psr_far01:.8f}, PSR in FAR@0.01: {psr_far001:.8f}, PSR in FAR@0.001: {psr_far0001:.8f}\n"
    print(result_str)
    with open(result_fn, 'a') as f:
        f.write(result_str)
    f.close()